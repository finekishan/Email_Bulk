...
app.post("/register", async (c) => {
  try {
    ...
    try {
      const userId = await mongoDatabase.createUser(email, name, password);
      const token = await mongoDatabase.createSession(userId);
...
app.post("/login", async (c) => {
  try {
    ...
    const token = await mongoDatabase.createSession(user.id);
...
app.post("/logout", async (c) => {
  try {
    ...
    if (token) {
      mongoDatabase.deleteSession(token);
    }
